...
.comment-improvement-button{
    display: var(--display-comment-improvement-button);
    background: none;
    border: none;
    margin: 0px 5px;
    padding: 0px;
...
.comment-improvement-button:hover{
    cursor: pointer;
}
.ChatMessage_chatMessage__2wev4{
    --display-comment-improvement-button: none;
}
.ChatMessage_chatMessage__2wev4:hover{
    --display-comment-improvement-button: inline-box;
...
const Zones2CN = new Map([
    ["Party: Smelly Planet", "队伍：臭臭星球"],
    ["Party: Swamp Planet", "队伍：沼泽星球"],
    ["Party: Aqua Planet", "队伍：海洋星球"],
    ["Party: Jungle Planet", "队伍：丛林星球"],
    ["Party: Gobo Planet", "队伍：哥布林星球"],
    ["Party: Planet Of The Eyes", "队伍：眼球星球"],
    ["Party: Sorcerer's Tower", "队伍：法师塔"],
    ["Party: Bear With It", "队伍：熊熊星球"],
    ["Party: Golem Cave", "队伍：魔像洞穴"],
    ["Party: Twilight Zone", "队伍：暮光之地"],
    ["Party: Infernal Abyss", "队伍：地狱深渊"],
    ["Party: Chimerical Den", "地下城：奇幻洞穴"],
    ["Party: Sinister Circus", "地下城：阴森马戏团"],
    ["Party: Enchanted Fortress", "地下城：秘法要塞"],
    ["Party: Pirate Cove", "地下城：海盗基地"],
    ["Party: Infernal Abyss (Elite)", "队伍：地狱深渊（精英）"],
    ["Party: Smelly Planet (Elite)", "队伍：臭臭星球（精英）"],
    ["Party: Swamp Planet (Elite)", "队伍：沼泽星球（精英）"],
    ["Party: Aqua Planet (Elite)", "队伍：海洋星球（精英）"],
    ["Party: Jungle Planet (Elite)", "队伍：丛林星球（精英）"],
    ["Party: Gobo Planet (Elite)", "队伍：哥布林星球（精英）"],
    ["Party: Planet Of The Eyes (Elite)", "队伍：眼球星球（精英）"],
    ["Party: Sorcerer's Tower (Elite)", "队伍：法师塔（精英）"],
    ["Party: Bear With It (Elite)", "队伍：熊熊星球（精英）"],
    ["Party: Golem Cave (Elite)", "队伍：魔像洞穴（精英）"],
    ["Party: Twilight Zone (Elite)", "队伍：暮光之地（精英）"],
    ["Party: Infernal Abyss (Elite)", "队伍：地狱深渊（精英）"],
    ["Party: Infernal Abyss (Elite)", "队伍：地狱深渊（精英）"],
]);
const TranslatePartyLink = () => {
    const flag = new Set();
    document.querySelectorAll("div.PartyLink_partyLink__3yDta:not([translated])").forEach(div => {
        div.setAttribute("translated", "");
        const translated = Zones2CN.get(div.textContent);
        if(translated) div.textContent = translated;
        /*
        if(translated === "队伍：暮光之地"){
            flag.add("有新的暮光之地组队");
        }
        if(translated === "队伍：魔像洞穴（精英）"){
            flag.add("有新的精石组队");
        }
        */
    });
    [...flag.keys()].forEach(info => new Notification(info));
}
...
const WatchedChannels = new Set([
    "/chat_channel_types/guild","/chat_channel_types/whisper","/chat_channel_types/party"
]);
window.WebSocket = function (...args) {
    ...
    ws.addEventListener('message', (event) => {
        ...
        try{
            ...
            if(!obj.channelTypeHrid) obj.channelTypeHrid = obj.chan;
            switch(obj.type){
                //{"type":"announcement_updated","announcementMessage":"sorry about the frequent restarts. hopefully issue should be resolved now.","timestamp":"2025-04-18T02:48:12.724412166Z"}
                ...
                case "chat_message_received":{
                    if(obj.message.isSystemMessage) return;
                    if(obj.message.isModMessage){
                        new Notification(`管理员的发言：`, {body: obj.message.m});
                    }
                    if(obj.message.chan === "/chat_channel_types/guild" && obj.message.sName !== UserStat.character.name){
                        new Notification(`${obj.message.sName}: `, {body: obj.message.m, })
                    }
                    if((obj.message.chan === "/chat_channel_types/whisper" || obj.message.chan === "/chat_channel_types/party") && obj.message.sName !== UserStat.character.name){
                        new Notification(`${obj.message.sName}: `, {body: obj.message.m, requireInteraction: !document.hasFocus()});
                    }
                    if(UserStat.character.gameMode === "standard") IDBAdd("ChatMessage", obj.message);
                    break;
                }
                ...
                case "battle_updated":{
                    if(Flags.outofMana){
                        if(obj.battleId === 1) Flags.outofMana = false;
                        else if(obj.battleId % 10 == 0 && Object.entries(obj.mMap).filter(([index, status]) => status.cHP < 200).length > 0){
                            if(!Flags.warnedOutOfMana) new Notification("蓝量曾经耗尽，boss战即将结束", {requireInteraction: !document.hasFocus()});
                            Flags.warnedOutOfMana = true;
                        }
                        else Flags.warnedOutOfMana = false;
                    }
                    else{
                        if(Object.entries(obj.pMap).filter(([index, status]) => index == 0 && status.isAutoAtk && status.cMP < 100).length > 0){
                            if(!Flags.outofMana) new Notification("我方自动攻击", {requireInteraction: !document.hasFocus()});
                            Flags.outofMana = true;
                        }
                    }
                }
            }
